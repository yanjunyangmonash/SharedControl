...
def tool_or_noise_r(rightTool_groups, pre_rightToolTot, num_frozen_frames, pre_twoTools_touch):
    ...
    if len(rightTool_groups) > 2:
        ...
        elif pre_rightToolTot > 22000:
            rightToolTot_area = pre_rightToolTot
            right_changerate = 0
        # Detect normal tool withdrawal
        # Use 10000 for clip 16
        elif pre_rightToolTot < 20000:
...
if __name__ == "__main__":
    ...
    for frames in range(5871, 5927, 1):
        ...
        cv2.putText(frame1, "Left Tool area change rate: {:.2f}%".format(leftchangerate), (20, 80), cv2.FONT_ITALIC,
                    0.5,
                    (0, 255, 0))
        ...
        cv2.putText(frame1, "Right Tool area change rate: {:.2f}%".format(rightchangerate), (650, 80), cv2.FONT_ITALIC,
                    0.5,
                    (0, 255, 0))

        cv2.imwrite('Clip16_GUITest1/clip25' + '_' + str(frames) + '.jpg', frame1)
        cv2.waitKey(0)

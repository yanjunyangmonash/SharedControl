...
###################################################################################
############## Use k value to get the edge point instead of distance ##############
###################################################################################
...
def get_feature_points_b_use_k(centers_matrix, bottom_or_top=1):
    # Situation b is for bottom&curve and top&curve boundary
    list_points_x = sorted(centers_matrix, key=lambda s: s[0])
    list_points_y = sorted(centers_matrix, key=lambda s: s[1])
    right_point = (list_points_x[-1][0], list_points_x[-1][1])
    ...
    mid_point = ((right_point[0] + top_or_bottom_point[0]) / 2, (right_point[1] + top_or_bottom_point[1]) / 2)
    base_vector = ((right_point[0] - top_or_bottom_point[0]), (right_point[1] - top_or_bottom_point[1]))

    list_interval_angle = []
    for i in range(len(centers_matrix)):
        if centers_matrix[i][0] == right_point[0] or centers_matrix[i][0] == top_or_bottom_point[0]:
            list_interval_angle.append(0)
            continue
        dx1 = base_vector[0]
        dy1 = base_vector[1]
        ...
        list_interval_angle.append(included_angle)
    sorted_list_interval_angle = sorted(list_interval_angle)
    next_edge_number = list_interval_angle.index(sorted_list_interval_angle[-1])
    next_edge_point = (centers_matrix[next_edge_number][0], centers_matrix[next_edge_number][1])
    next_right_number = list_interval_angle.index(sorted_list_interval_angle[2])
    next_right_point = (centers_matrix[next_right_number][0], centers_matrix[next_right_number][1])

    return right_point, next_right_point, top_or_bottom_point, next_edge_point


def get_feature_points_c_use_k(centers_matrix, bottom_or_top=1):
    # Situation c is for bottom and top boundary
    if bottom_or_top:
        list_points_y = sorted(centers_matrix, key=lambda s: s[1], reverse=True)
        bottom_points = [list_points_y[0], list_points_y[1], list_points_y[2]]
        ...
        mid_to_right = list_bottom_points_x[0][0] - list_bottom_points_x[1][0]
        mid_to_left = list_bottom_points_x[1][0] - list_bottom_points_x[2][0]
        ...
        bottom_point = (list_bottom_points_x[1][0], list_bottom_points_x[1][1])

        mid_point = ((right_bottom_point[0] + bottom_point[0]) / 2, (right_bottom_point[1] + bottom_point[1]) / 2)
        base_vector = ((right_bottom_point[0] - bottom_point[0]), (right_bottom_point[1] - bottom_point[1]))

        list_interval_angle = []
        for i in range(len(centers_matrix)):
            if centers_matrix[i][0] == right_bottom_point[0] or centers_matrix[i][0] == bottom_point[0]:
                list_interval_angle.append(0)
                continue
            dx1 = base_vector[0]
            dy1 = base_vector[1]
            ...
            list_interval_angle.append(included_angle)
        sorted_list_interval_angle = sorted(list_interval_angle)
        next_bottom_number = list_interval_angle.index(sorted_list_interval_angle[-1])
        next_bottom_point = (centers_matrix[next_bottom_number][0], centers_matrix[next_bottom_number][1])
        next_right_number = list_interval_angle.index(sorted_list_interval_angle[2])
        next_right_bottom_point = (centers_matrix[next_right_number][0], centers_matrix[next_right_number][1])
        ...
        if mid_to_right < mid_to_left:
            ...
            right_top_point = (list_top_points_x[0][0], list_top_points_x[0][1])
            del list_top_points_x[0]
        top_point = (list_top_points_x[1][0], list_top_points_x[1][1])

        mid_point = ((right_top_point[0] + top_point[0]) / 2, (right_top_point[1] + top_point[1]) / 2)
        base_vector = ((right_top_point[0] - top_point[0]), (right_top_point[1] - top_point[1]))

        list_interval_angle = []
        for i in range(len(centers_matrix)):
            if centers_matrix[i][0] == right_top_point[0] or centers_matrix[i][0] == top_point[0]:
                list_interval_angle.append(0)
                continue
            dx1 = base_vector[0]
            dy1 = base_vector[1]
            ...
            list_interval_angle.append(included_angle)
        sorted_list_interval_angle = sorted(list_interval_angle)
        next_top_number = list_interval_angle.index(sorted_list_interval_angle[-1])
        next_top_point = (centers_matrix[next_top_number][0], centers_matrix[next_top_number][1])
        next_right_number = list_interval_angle.index(sorted_list_interval_angle[2])
        next_right_top_point = (centers_matrix[next_right_number][0], centers_matrix[next_right_number][1])
